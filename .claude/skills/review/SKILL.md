...
- `css/style.css` — CSS 품질 및 반응형
- `js/render.js` — 데이터 렌더링 로직
- `js/main.js` — 인터랙션 및 애니메이션
- `data/*.json` — JSON 데이터 유효성 및 구조

## 프로젝트 컨텍스트

- 빌드 도구 없는 순수 정적 사이트
- `data/*.json` → `render.js` → `siteDataReady` 이벤트 → `main.js` 순서로 동작
